...
public class Fact_HOUSE_HEIGHT_WITHIN_NORM extends Fact {
	...
	private void evaluateValue(ServerGlobals theServerGlobals, Case theCase, AssessRequestReply theReply, int level,
		...
		if (dp_Purpose_HM_OBJECT_HEIGHT.hasValue()) {
			...
			if (objHeight <= norm ){
				withinLimit = true;
				valueKnown = true;
				this.addToExplanation("Hoogte woning ["+ roundedHeight+ "] valt binnen norm van: " + roundedNorm + " meter");
			} else {
				withinLimit = false;
				valueKnown = true;
				this.addToExplanation("Hoogte woning ["+ roundedHeight+ "] valt BUITEN de norm van: " + roundedNorm + " meter");
			} 
