		...
		if (dp_Purpose_HM_OBJECT_HEIGHT.hasValue()) {
			...
			if (objHeight <= norm) {
				...
				this.addToExplanation(
						"Hoogte kantoor [" + roundedHeight + "] valt BINNEN norm van: " + roundedNorm + " meter");
			} else {
				withinLimit = false;
				valueKnown = true;
				this.addToExplanation(
						"Hoogte kantoor [" + roundedHeight + "] valt BUITEN de norm van: " + roundedNorm + " meter");
			}
